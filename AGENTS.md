...
Auto-Merge Rule (captain-approved, added 2026-09-21): self-merge a shared tracked-material PR in this repo once it is green, mirroring a project's `yolo` posture. This is a standing decision, committed in advance, not a permission negotiated in the moment of any one merge. It does not override a harness's own tool-level restrictions - when the acting session runs on a harness that itself blocks self-merge (for example, Claude Code's own Auto-Mode classifier, which gates the merge call independent of this policy), a human executes that merge call instead; the policy governs firstmate's decision to attempt the merge, not the tool-level gate. The red-merge, destructive, irreversible, and security-sensitive boundaries in hard rule 2 remain independently in force.
